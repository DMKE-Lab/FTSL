...
class Trainer(object):
    ...
    def build_connection(self, max_=100):
        ...
        # self.e1_rele2中存储的是实体的邻居关系id和邻居实体id
        self.e1_rele2 = defaultdict(list)
        # self.e1_degrees中存储的是实体id对应的邻居节点的数量（最大是max_neighbor个）
    ...
    def train(self):
        ...
        for data in train_generate(self.dataset, self.batch_size, self.train_few, self.symbol2id, self.ent2id,
            ...
            # support, query, false, support_left, support_right, query_left, query_right, false_left, false_right, support_t, query_t, false_t = data
            ...
            # todo time_list----->tensor

            # ## TATtansE
            # timeid2vec = json.load(open('./icews18' + '/timeid2vec.json'))
            # support_t = [timeid2vec[str(time)] for time in support_t]
            # query_t = [timeid2vec[str(time)] for time in query_t]
            # false_t = [timeid2vec[str(time)] for time in false_t]

            # ##TTransE
            # time2id = json.load(open('./icews18' + '/time2id.json'))
            # support_t = [time2id[str(time)] for time in support_t]
            # query_t = [time2id[str(time)] for time in query_t]
            # false_t = [time2id[str(time)] for time in false_t]


            # support_t = Variable(torch.LongTensor(support_t)).to(self.device)
            # query_t = Variable(torch.LongTensor(query_t)).to(self.device)
            # false_t = Variable(torch.LongTensor(false_t)).to(self.device)
        ...
        for query_ in test_tasks.keys():
            ...
            for triple in test_tasks[query_][few:]:
                ...
                if meta:
                    ...
                    # todo 改变eval中的matcher参数
                    # scores, _ = self.Matcher(support, query, false=None, isEval=True,
                    #                                           support_meta=support_meta,
                    #                                           query_meta=query_meta,
                    #                                           false_meta=None,
                    #                                           support_t=support_t,
